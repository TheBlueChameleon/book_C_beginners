...
Expressions may become arbitrarily complex in C. We are not limited to a mere two \emph{arguments} (like in \inC{x + 2}. The variable \texttt{x} and the constant \inC{2} are arguments), but can evaluate entire chains of operations. The expression \inC{x + 1.2 - 5.0 + 2.2 * x - y / 7.3} is valid in C. For the most part, these expressions are parsed from the left to the right (\inC{x + 1.2} is evaluated before subtracting \inC{5.0}). However, some operations have higher priority than others. Multiplication and division are always evaluated before addition/subtraction, as is custom in \enquote{human maths}, too. You may use parentheses \texttt{()} to deviate from this behaviour. Hence, \inC{2 + 5 * 8} will give you \inC{42} 
while \texttt{(2 + 5) * 8} will give you \inC{56}.
...
A rarely used but nonetheless present oddity of the C programming language is that value assignments (\texttt{variable = expression}) themselves are considered \enquote{computations} and have a result (in the sense of \inC{1 + 2} has a result, which is \inC{3}). The usually intended update of a variable is formally only a \emph{side effect}. The \emph{result} of this \enquote{assignment computation} is the value of the assigned expression.
...
\end{minted}
\captionof{code}{Assignment as result}
\end{codebox}
...
\begin{defbox}[Dissection of the chain assignment]
\begin{align*}
	\underbrace{
		\texttt{a = }
		\underbrace{
	...
	\texttt{;}
\end{align*}
\captionof{figure}{Evaluation of a chain assignment}

Since multiplication takes highest precedence, the subexpression \inC{5 * 8} is evaluated first and gives \inC{40}. Subsequently, \inC{2 + 40} is evaluated to \inC{42}. The third step then is assigning this value \inC{42} to the variable \texttt{d}, which in itself has the result \inC{42}. The steps four through 7 act on different variables, but otherwise are the same in principle. The last result (the assignment to \texttt{a}) is discarded. That means, the side effect (updating \texttt{a}) is executed, but the result \inC{42} lingers in the processor for a nanosecond before it is \enquote{forgotten}.
\end{defbox}
...
\subsection{Controlling Length and Alignment of the Output}
...
\end{tikzpicture}
\end{center}
\captionof{figure}{Structure elements in the declaration of a pointer}
\end{defbox}
...
\begin{hintbox}[One declaration per line]
...
For example, in code \ref{code:divisions}, we could in principle get away with two lines of code: one for the \inC{int}s and one for the \texttt{double}s. However, it is commonly regarded preferable to stick with the one declaration per line rule. Here, line means the entire protion of code between two semicolons. So code like
...
We would expect here that both, \texttt{foo} and \texttt{bar} are of type \inC{int*}. In fact, this only holds for \texttt{foo}; \texttt{bar} will actually be of type \inC{int}, \ie no pointer. If we want both of them to be pointers, we need to type:
\begin{warnbox}[Spurious declaration of pointers, leftupper=7mm]
\begin{minted}[linenos]{c}
int main () {
    int* foo, *bar;
...
\begin{codebox}[Spurious declaration of pointers]
\begin{minted}[linenos]{c}
int main () {
    int* foo;
    int* bar;
...
\end{minted}
\captionof{code}{Incrementing a variable -- long version}
\end{codebox}
...
\end{minted}
\captionof{code}{Incrementing a variable -- shorthand version}
\end{codebox}
...
\end{minted}
\captionof{code}{Shorthand versions} \label{code:shorthandsAll}
\end{codebox}

In code \ref{code:shorthandsAll}, the variable \texttt{i} evolves as follows:
\begin{center}
\newcolumntype{L}{>{\centering         \arraybackslash}p{.15\linewidth}}
\newcolumntype{V}{>{\centering\ttfamily\arraybackslash}m{.15\linewidth}}
\newcolumntype{N}{>{\centering         \arraybackslash}p{.30\linewidth}}
...
\tabcrlf
	 ...
	 8 & 12 & \makecell{multiplying by an expression \\ \texttt{j + 1} evaluates to \texttt{2}} \\
	 9 &  6 & \makecell{multiplying by an expression \\ \texttt{i - 10} evaluates to \texttt{2}} \\
...
Take particular note of the oder of events in line 9: first, the right hand side of the equals sign is evaluated. That means, \texttt{i} refers to the value \emph{before} it is overwritten, and consequently gives \inC{10}. Hence, the right hand side \inC{i - 10} becomes \inC{12 - 10} becomes \inC{2}. Only \emph{after} the right hand side is completely evaluated, the update part (computing the new value of \texttt{i} and overwriting \texttt{i}) is executed: the computer computes \inC{i / 2} and eventually stores this result in \texttt{i}.
...
\end{minted}
\captionof{code}{Catching the results of increment and decrement operator (prefix form)}
\end{codebox}
...
\end{minted}
\captionof{code}{Catching the results of increment and decrement operator (postfix form)}
\end{codebox}

\begin{hintbox}[Shorthand forms and pointers]
...
\begin{hintbox}[The KISS principle]
...
\begin{itemize}
\item contains a variable storing the value \inC{3.141592654}
\item contains a pointer to that value
\item prints that variable's value using the pointer
\end{itemize}

\subsection*{Debugging}
The expected output of this program is \inC{7.5}. However, something went wrong. Fix the mistake(s).
...
Note:  We haven't discussed the type \inC{char*} in detail. For now it's sufficient to know that you can use the format string \texttt{\%s} to print the text at this pointer's location. Format string modifications work here as usual.
\begin{codebox}[exo3-3.c]
\begin{minted}[linenos]{c}
#include <stdio.h>
...
Changes made:
\begin{itemize}
\item Change the data type of \texttt{result} to \inC{double}
...
\emph{By the way:}\\
Did you have trouble following the evolution of values? I wouldn't blame you -- the code example isn't quite \emph{good code}. It was designed to contain as many turns and indirections as possible to train you to deal with them \emph{when the need arises}. While we need to be familiar with these concepts to realize some feats, it is best to keep your code as \emph{down to earth} as possible. Avoid constructs where several things happen at once (\eg \texttt{z = result++;}). Also: good names save lives! In your projects, you will not just have \enquote{raw numbers}, but usually values with \emph{meaning} (like \texttt{numberOfDogs}). Being reminded of that context does wonders when keeping track of the flow of information.

\subsection*{Solution: Design}
\begin{codebox}[exo3-4.c]
\begin{minted}[linenos]{c}
#include <stdio.h>
